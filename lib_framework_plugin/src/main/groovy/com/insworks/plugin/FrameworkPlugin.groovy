...
class FrameworkPlugin implements Plugin<Project> {
    ...
    //lib配置json默认名称
    public String libJsonName = subLibName
    //模块根目录默认名称
    public String subModuleName = "androidModule"
    //模块配置json默认名称
    public String moduleJsonName = subModuleName
    ...
    void apply(Project project) {
        ...
        println("=================================")
        //向project中添加frame节点
        project.extensions.add("frame", FlavorType)
        ...
        if (!project.android) {
            ...
            if (isApp(project)) {
                ...
                //插件里暂时不用  gradle中需要 否则子模块无法找不到aar文件
//                mProject.repositories.flatDir {
//                    //引用所有libs 用于aar文件
//                    DirOption.findAllLibsDir(mProject, subModuleName).each { f ->
//                        dirs f.absoluteFile
//                    }
//                }
            }
